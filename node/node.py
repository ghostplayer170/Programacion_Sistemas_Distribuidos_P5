...
registry_url = os.getenv('REGISTRY_URL', 'http://registry:8080') # Defaulting to http://registry:8080 if REGISTRY_URL is not set
nodes = [] # List of nodes
...
def check_current_coordinator(): # if current coordinator id is less than the node id, then start the election
    ...
    if current_coordinator is not None and current_coordinator < node_id:
        log_message(f"Node {node_id} detected that it has a higher ID than the current coordinator {current_coordinator}. Initiating election.")
        initiate_election()
